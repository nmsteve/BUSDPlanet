...
contract BusdPlanetDividendTracker is Ownable, DividendPayingToken, ERC20TokenRecover, IBusdPlanetDividendTracker {
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount)
        ...
        onlyOwner
    {
        require(tokenAddress != dividendToken, "BusdPlanetDividendTracker: Cannot retrieve USDT");
    ...
    function _transfer(
        ...
        require(false, "BusdPlanetDividendTracker: No transfers allowed");
    }

    function withdrawDividend() public pure override(DividendPayingToken, IDividendPayingTokenInterface) {
        require(
            false,
            "BusdPlanetDividendTracker: Disabled. Use the 'claim' function on the main TetherMoon contract."
        );
    }

    function excludeFromDividends(address account) external override onlyOwnerOrDeployer {
        require(!excludedFromDividends[account], "BusdPlanetDividendTracker: Account already excluded");
    ...
    function includeInDividends(address account) external override onlyOwnerOrDeployer {
        require(excludedFromDividends[account], "BusdPlanetDividendTracker: Account not excluded");
